...
static public class MeshSimplification
{
    ...
    {
        Vector3[] verts = mesh.vertices;
        int origNumVerts = verts.Length;

        List<Vertice> workingSet = new List<Vertice>(origNumVerts);
                ...
                if (a.edges != null) foreach (Edge ac in a.edges)
                    {
                        ...
                        if (a.CanFollow(ac, out c))
                        {
                            ...
                            foreach (Edge edge_of_a in a.edges)
                            {
                                if (edge_of_a != ac)
                                {
                                    Vertice o = edge_of_a.GetOpposite(a);
        ...
        List<Vertice> simplifiedVerts = new List<Vertice>();
        foreach (Vertice v in workingSet) if (v.edges != null)
                simplifiedVerts.Add(v);

        int simplifiedNumVerts = simplifiedVerts.Count;
        Vector3[] newPositions = new Vector3[simplifiedNumVerts];
        ...
        HashSet<Face> triSet = new HashSet<Face>();
    ...
    private class Vertice
    {
        ...
        public bool CanFollow(Edge transportEdge, out Vertice opposite)
        {
            ...
            HashSet<Face> localTris = new HashSet<Face>();
            ...
            Vector3 targetPos = opposite.position;
            HashSet<Face>.Enumerator lTriEnum = localTris.GetEnumerator();
            try
            {
    ...
    private class Edge
    {
        ...
        public bool HasEqualPlanes()
        {
            ...
            Face f0 = faces[0];
            Edge f0e0 = faces[0].edges[0];

            Face f1 = faces[1];
            Edge f1e0 = faces[1].edges[0];

            Edge e0 = f0e0 != this ? f0e0 : f0.edges[1];
            Edge e1 = f1e0 != this ? f1e0 : f1.edges[1];

            Vector3 v0 = vertices[1].position - vertices[0].position;
            Vector3 v1 = e0.vertices[1].position - e0.vertices[0].position;
            Vector3 v2 = e1.vertices[1].position - e1.vertices[0].position;

            Vector3 n0 = Vector3.Cross(v0, v1);

            float dot = Vector3.Dot(n0, v2);
    ...
    private class Face
    {
        ...
        {
            Vertice o0 = o.vertices[0];
            Vertice o1 = o.vertices[1];
            for (int i = 0; i < 3; i++)
        ...
        public bool MoveWouldFlip(Vertice v, Vector3 p)
        {
            ...
            Vector3 ov0 = oppositeEdge.vertices[0].position;
            Vector3 ov = oppositeEdge.vertices[1].position - ov0;
            Vector3 ot = p - ov0;
            Vector3 ct = v.position - ov0;

            Vector3 cross0 = Vector3.Cross(ot, ov);
            float c0SqrMagnitude = cross0.sqrMagnitude;
            if (c0SqrMagnitude < 0.0001f) return true;

            Vector3 cross1 = Vector3.Cross(ct, ov);
            float c1SqrMagnitude = cross1.sqrMagnitude;
